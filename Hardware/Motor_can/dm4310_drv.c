...
void dm4310_fbdata_1(motor_t *motor, uint8_t *rx_data)
{
	motor->para.id = (rx_data[0])&0x0F;
	motor->para.state = (rx_data[0])>>4;
	motor->para.p_int=(rx_data[1]<<8)|rx_data[2];
	motor->para.v_int=(rx_data[3]<<4)|(rx_data[4]>>4);
	motor->para.t_int=((rx_data[4]&0xF)<<8)|rx_data[5];
	motor->para.pos = uint_to_float(motor->para.p_int, -12.5, 12.5, 16); // (-12.5,12.5)
	motor->para.vel = uint_to_float(motor->para.v_int, -30, 30, 12); // (-45.0,45.0)
	motor->para.tor = uint_to_float(motor->para.t_int, -5, 5, 12);  // (-18.0,18.0)
	motor->para.Tmos = (float)(rx_data[6]);
	motor->para.Tcoil = (float)(rx_data[7]);
}
